...
const CardBetDetail = ({
	...
	<Card>
		<Row title="SPORT" detail={sportRename[sport].toUpperCase()} />
		<Hr backgroundColor={colors.dark} height={1} />
		<Row title="LEAGUE" detail={league} />
		<Hr backgroundColor={colors.dark} height={1} />
		<Row
			title="TIME"
			detail={moment(matchTime).tz('America/Los_Angeles').format('ddd, MMM DD - hh:mm A').toUpperCase()}
		/>
		<Hr backgroundColor={colors.dark} height={1} />
		...
		<RowTeam
			title="HOME"
			teamName={team.home.name.toUpperCase()}
			sport={sport}
			score={score.home}
			status={status}
		/>
		...
		<RowTeam
			title="AWAY"
			teamName={team.away.name.toUpperCase()}
			sport={sport}
			score={score.away}
			status={status}
		/>
